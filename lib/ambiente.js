...
// Codigo de instalacao da nuvem: a variavel SENHA_PAINEL ou, se ela nao existir,
// um codigo aleatorio gerado na primeira partida, guardado no volume e mostrado
// so nos logs do Railway (que so o dono da conta ve).
function codigoInstalacao() {
  if (process.env.SENHA_PAINEL) return process.env.SENHA_PAINEL;
  if (!NUVEM) return '';
  const arq = path.join(DADOS, 'codigo-instalacao.txt');
  try { return fs.readFileSync(arq, 'utf8').trim(); } catch (e) { /* primeira vez */ }
  const codigo = require('crypto').randomBytes(12).toString('base64url');
  fs.mkdirSync(DADOS, { recursive: true });
  fs.writeFileSync(arq, codigo, { mode: 0o600 });
  return codigo;
}
const SENHA = codigoInstalacao();
