...
function acharMomentos(s, duracao, conteudo = 'rp') {
  ...
  for (const m of [...momentos].sort((a, b) => b.nota - a.nota)) {
    ...
    if (total >= ALVO || total + d > MAX || m.nota < 45) { m.selecionado = false; continue; }
    m.selecionado = true;
    total += d;
  }
...
function sugerir(job, prefs = {}) {
  ...
  else if (f && t) titulo = 'FUGA E TROCA DE TIRO 🔥';
  else if (f) titulo = (f > 1 ? 'FUGI DA POLÍCIA ' + f + ' VEZES' : 'FUGA INSANA DA POLÍCIA') + ' 🚔';
  else if (t) titulo = 'TROCA DE TIRO 🔫';
  else titulo = 'OS MELHORES MOMENTOS DA LIVE 🔥';
  const final = String(prefs.estudioFinalTitulo || '').trim();
  if (final) titulo = (titulo + ' ' + (final.startsWith('|') ? final : '| ' + final)).slice(0, 100);
  let pos = 0;
  const numero = {};
  ...
  const partes = [
    job.conteudo === 'react' ? 'As melhores reações da live "' + (job.titulo || '') + '".' : 'Os melhores momentos da live "' + (job.titulo || '') + '".',
    '',
    capitulos.join('\n'),
    '',
    prefs.chamada || '',
...
const TEXTOS_THUMB = {
  ...
  tiro: [['TROCA DE', 'TIRO'], ['TIROTEIO', 'PESADO'], ['NINGUÉM', 'SOBREVIVEU?']],
...
async function montarThumb(job, quadro, saida, { linha1, linha2, estilo, cam }) {
  ...
  const etiqueta = String(lerPrefs().estudioEtiqueta == null ? (job.conteudo === 'react' ? 'REACT' : 'GTA RP') : lerPrefs().estudioEtiqueta).trim().toLocaleUpperCase('pt-BR').slice(0, 30);
  fs.writeFileSync(path.join(dir, 'tag.txt'), etiqueta || ' ', 'utf8');
  ...
  const filtro = [
    ...
    'color=c=black@0:s=1280x720:d=1,format=rgba,' +
      (etiqueta ? `drawtext=fontfile='${FONTE_TAG}':textfile=tag.txt:fontsize=34:fontcolor=white:box=1:boxcolor=black@0.85:boxborderw=12:x=58:y=52,` : '') +
...
async function gerarThumbs(job, { textos, tempo } = {}) {
  const dir = pastaJob(job.id);
  ...
  const rostoNormal = Number.isFinite(Number(tempo)) && tempo !== null && tempo !== '' ? null : await rostoBase(job, url, cam);
  const thumbs = [];
  for (let k = 0; k < 3; k++) {
    const m = base[k % base.length];
    job.etapa = 'procurando sua melhor reação no momento ' + (k + 1);
    const escolhido = Number.isFinite(Number(tempo)) && tempo !== null && tempo !== '';
    const quadro = escolhido
      ? await quadroEm(url, Number(tempo), path.join(dir, 'escolhido-' + Math.round(Number(tempo) * 10) + '.jpg'))
      : await melhorQuadro(job, url, m, cam, rostoNormal);
...
async function refazerThumbs(id, { textos, tempo } = {}) {
  const job = obter(id);
  await gerarThumbs(job, { textos, tempo });
  job.etapa = null;
  salvar(job);
  return job;
}
