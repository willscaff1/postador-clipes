...
// ---------- admin padrao ----------
// Sem nenhum usuario, cria o perfil do dono: usuario "admin", senha "admin" no PC
// (so abre no proprio computador) ou o codigo SENHA_PAINEL na nuvem. O primeiro
// login obriga a trocar a senha.
function garantirAdmin() {
  if (listaUsuarios().length) return null;
  const senhaInicial = ambiente.NUVEM ? ambiente.SENHA : 'admin';
  if (!senhaInicial) return null;
  const u = {
    id: crypto.randomBytes(6).toString('hex'), usuario: 'admin', nome: 'Willian Scaff', papel: 'admin',
    criadoEm: new Date().toISOString(), trocarSenha: true, ...hashSenha(senhaInicial),
  };
  listaUsuarios().push(u);
  salvarUsuarios();
  return u;
}
const adminPadraoIntocado = () => listaUsuarios().length === 1 && listaUsuarios()[0].trocarSenha && listaUsuarios()[0].usuario === 'admin';

// Troca obrigatoria (primeiro login): nome, usuario e senha nova.
function concluirTrocaObrigatoria(quem, { nome, usuario, nova, repete }) {
  if (nova !== repete) throw new Error('As duas senhas nao batem.');
  validarSenha(nova);
  if (senhaConfere(quem.usuario, nova)) throw new Error('A senha nova tem que ser diferente da atual.');
  const login = validarUsuario(usuario || quem.usuario.usuario);
  if (login !== quem.usuario.usuario && listaUsuarios().some((x) => x.usuario === login)) throw new Error('Ja existe um usuario "' + login + '".');
  quem.usuario.usuario = login;
  if (nome && String(nome).trim()) quem.usuario.nome = String(nome).trim().slice(0, 60);
  Object.assign(quem.usuario, hashSenha(nova));
  delete quem.usuario.trocarSenha;
  salvarUsuarios();
  sessoes = listaSessoes().filter((x) => x.usuarioId !== quem.usuario.id || x.id === quem.sessao.id);
  salvarSessoes(true);
}
...
function publico(u) {
  return { id: u.id, usuario: u.usuario, nome: u.nome, papel: u.papel, criadoEm: u.criadoEm, ultimoAcesso: u.ultimoAcesso || null, trocarSenha: !!u.trocarSenha };
}
...
function atualizarUsuario(quem, id, { nome, papel, novaSenha }) {
  ...
  if (novaSenha) {
    ...
    if (u.id !== quem.usuario.id) u.trocarSenha = true;
...
function pagina({ erro, primeiro, troca, quem } = {}) {
  const esc = (s) => String(s).replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
  const campoSenha = (nome, rotulo, auto) => `<label>${rotulo}<span class="senha"><input type="password" name="${nome}" autocomplete="${auto}" required minlength="${primeiro || troca ? 8 : 1}"><button type="button" class="olho" aria-label="Mostrar senha">👁</button></span></label>`;
  const dica = !primeiro && !troca && adminPadraoIntocado()
    ? '<div class="dica-box">Primeiro acesso: usuário <b>admin</b> e senha <b>' + (ambiente.NUVEM ? 'o código SENHA_PAINEL do Railway' : 'admin') + '</b>. Logo depois você cria sua senha.</div>' : '';
  const corpo = troca
    ? `<h1>Crie sua senha</h1><p>Antes de começar, troque a senha provisória. Se quiser, mude também seu nome e usuário.</p>
      ${erro ? `<div class="erro">${esc(erro)}</div>` : ''}
      <form method="post" action="/trocar-senha">
        <label>Seu nome<input name="nome" value="${esc(quem.usuario.nome)}" required></label>
        <label>Usuário<input name="usuario" value="${esc(quem.usuario.usuario)}" required pattern="[A-Za-z0-9._-]{3,32}" autocomplete="username"></label>
        ${campoSenha('nova', 'Nova senha (mínimo 8)', 'new-password')}
        ${campoSenha('repete', 'Repita a nova senha', 'new-password')}
    ...
    : `<h1>Postador de Clipes</h1><p>Entre com seu usuário e senha.</p>
      ${erro ? `<div class="erro">${esc(erro)}</div>` : ''}${dica}
...
<title>${troca ? 'Crie sua senha' : primeiro ? 'Primeiro acesso' : 'Entrar'} · Postador de Clipes</title>
<style>
...
.dica-box{background:#241d3d;color:#c4b5fd;border-radius:10px;padding:9px 12px;font-size:13.5px;margin-bottom:14px}.dica-box b{color:#fff}
...
// Transferencia PC -> painel online: os usuarios vao com o hash da senha.
function exportarUsuarios() { return JSON.parse(JSON.stringify(listaUsuarios())); }
function importarUsuarios(lista) {
  const limpos = lista.map((u) => {
    if (!u || !/^[a-z0-9._-]{3,32}$/.test(u.usuario || '') || !/^[a-f0-9]{128}$/.test(u.hash || '') || !u.sal) throw new Error('Usuario invalido no pacote.');
    return { id: String(u.id), usuario: u.usuario, nome: String(u.nome || u.usuario).slice(0, 60), papel: u.papel === 'admin' ? 'admin' : 'editor', criadoEm: u.criadoEm, sal: String(u.sal), hash: u.hash, ultimoAcesso: u.ultimoAcesso };
  });
  if (!limpos.some((u) => u.papel === 'admin')) throw new Error('O pacote precisa ter pelo menos um administrador.');
  usuarios = limpos;
  salvarUsuarios();
  sessoes = [];
  salvarSessoes(true);
...
module.exports = {
  exportarUsuarios, importarUsuarios,
  garantirAdmin, concluirTrocaObrigatoria,
