...
const ESCOPOS = [
  ...
  // novos inscritos / quem saiu por dia
...
module.exports = {
  ...
  redirect: () => require('../ambiente').urlRetorno('youtube', { loopbackIp: true }),
  campos: [
  ...
  async metricas(cred, ctx, { ids = [] } = {}) {
    ...
    return {
      ...
      posts: (v.items || []).map((x) => ({
        ...
        duracao: (() => { const m = /PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?/.exec((x.contentDetails || {}).duration || ''); return m ? (Number(m[1]) || 0) * 3600 + (Number(m[2]) || 0) * 60 + (Number(m[3]) || 0) : null; })(),
      })).sort((a, b) => String(b.data).localeCompare(String(a.data))),
    };
  },
  // Novos inscritos e quem saiu, por dia (YouTube Analytics).
  async seguidoresPorDia(cred, ctx, dias = 30) {
    const token = await tokenValido(cred, ctx);
    const fim = new Date(); const ini = new Date(Date.now() - dias * 86400000);
    const d = (x) => x.toISOString().slice(0, 10);
    try {
      const { dados } = await pedir('https://youtubeanalytics.googleapis.com/v2/reports?' + new URLSearchParams({
        ids: 'channel==MINE', startDate: d(ini), endDate: d(fim), metrics: 'subscribersGained,subscribersLost', dimensions: 'day', sort: 'day',
      }), { rotulo: 'YouTube Analytics', headers: { Authorization: 'Bearer ' + token } });
      return { dias: (dados.rows || []).map(([dia, ganhos, perdas]) => ({ dia, ganhos, perdas })) };
    } catch (e) {
      if (/403|insufficient|scope|disabled|has not been used/i.test(e.message)) {
        return { dias: [], aviso: 'Pra ver novos inscritos e quem saiu: ative a "YouTube Analytics API" no Google Cloud e clique em Reconectar no cartao do YouTube.' };
      }
      throw e;
    }
  },

  // Comentarios recentes do canal (com as respostas).
  async comentarios(cred, ctx) {
    const token = await tokenValido(cred, ctx);
    const h = { Authorization: 'Bearer ' + token };
    const { dados: ch } = await pedir('https://www.googleapis.com/youtube/v3/channels?part=id,snippet&mine=true', { rotulo: 'YouTube', headers: h });
    const eu = ch.items && ch.items[0];
    if (!eu) return { comentarios: [] };
    const { dados } = await pedir('https://www.googleapis.com/youtube/v3/commentThreads?' + new URLSearchParams({
      part: 'snippet,replies', allThreadsRelatedToChannelId: eu.id, maxResults: '50', order: 'time', textFormat: 'plainText',
    }), { rotulo: 'YouTube', headers: h });
    const ids = [...new Set((dados.items || []).map((t) => t.snippet.videoId))];
    const titulos = {};
    if (ids.length) {
      const { dados: v } = await pedir('https://www.googleapis.com/youtube/v3/videos?part=snippet&id=' + ids.join(','), { rotulo: 'YouTube', headers: h });
      for (const x of v.items || []) titulos[x.id] = x.snippet.title;
    }
    return {
      comentarios: (dados.items || []).map((t) => {
        const c = t.snippet.topLevelComment.snippet;
        const respostas = ((t.replies && t.replies.comments) || []).map((r) => ({
          autor: r.snippet.authorDisplayName, texto: r.snippet.textDisplay, data: r.snippet.publishedAt, minha: r.snippet.authorChannelId && r.snippet.authorChannelId.value === eu.id,
        }));
        return {
          id: t.snippet.topLevelComment.id, autor: c.authorDisplayName, foto: c.authorProfileImageUrl, texto: c.textDisplay, data: c.publishedAt,
          curtidas: c.likeCount, post: titulos[t.snippet.videoId] || 'Vídeo', url: 'https://www.youtube.com/watch?v=' + t.snippet.videoId + '&lc=' + t.snippet.topLevelComment.id,
          respostas, respondido: respostas.some((r) => r.minha), meu: c.authorChannelId && c.authorChannelId.value === eu.id,
        };
      }),
      acoes: ['responder'],
    };
  ...
  async responderComentario(cred, ctx, id, texto) {
    const token = await tokenValido(cred, ctx);
    try {
      await pedir('https://www.googleapis.com/youtube/v3/comments?part=snippet', {
        method: 'POST', rotulo: 'YouTube', headers: { Authorization: 'Bearer ' + token }, json: { snippet: { parentId: id, textOriginal: texto } },
      });
    } catch (e) {
      if (/403|insufficient|scope/i.test(e.message)) throw new Error('Falta permissao pra responder: clique em Reconectar no cartao do YouTube.');
      throw e;
    }
  },

  // Views, minutos assistidos e inscritos por mes, ultimos 12 meses (YouTube Analytics).
  async mensal(cred, ctx) {
    const token = await tokenValido(cred, ctx);
    const hoje = new Date();
    ...
    try {
      const { dados } = await pedir('https://youtubeanalytics.googleapis.com/v2/reports?' + new URLSearchParams({
        ids: 'channel==MINE', startDate: d(ini), endDate: d(hoje), metrics: 'views,estimatedMinutesWatched,subscribersGained,subscribersLost', dimensions: 'month', sort: 'month',
      }), { rotulo: 'YouTube Analytics', headers: { Authorization: 'Bearer ' + token } });
      return { meses: (dados.rows || []).map(([mes, views, minutos, ganhos, perdas]) => ({ mes, views, minutos, ganhos, perdas })) };
    } catch (e) {
      if (/403|insufficient|scope|disabled|has not been used/i.test(e.message)) return { meses: [] };
      throw e;
    }
  },
